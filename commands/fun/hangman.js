...
module.exports = {
  ...
  execute: async (client, message, args, servers) => {
    ...
    if (servers[message.guild.id].word.includes(letter)) {
      ...
      if (servers[message.guild.id].letters.includes(letter))
        return message.channel.send(
          "YOU ALREADY GUESSED THAT LETTER CORRECTLY DUMBASS"
        );
      else {
        ...
        message.channel.send("", {
          files: [`./images/hangman_${servers[message.guild.id].strikes}.png`]
        });
        ...
        for (i = 0; i < servers[message.guild.id].word.length; i++) {
          ...
          for (j = 0; j < servers[message.guild.id].letters.length; j++) {
            if (
              servers[message.guild.id].letters[j] ==
              servers[message.guild.id].word.charAt(i)
            ) {
    ...
    // If letter guessed is not in word
      ...
      for (i = 0; i < servers[message.guild.id].word.length; i++) {
        ...
        if (!servers[message.guild.id].hit) str += "- ";
      }
      ...
      if (servers[message.guild.id].strikes == 6) {
        message.channel.send(
          `**HAHAHAHA YOU LOST! THE WORD WAS __${servers[
            message.guild.id
          ].word.toUpperCase()}__ DUMBASS!**`
        );
        servers[message.guild.id].word = random_word();
        servers[message.guild.id].letters = [];
        servers[message.guild.id].strikes = 0;
